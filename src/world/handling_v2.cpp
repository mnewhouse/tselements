  ...
  namespace world
  {
                                   ...
                                   double frame_duration)
    {
      ...
      for (auto& ws : wheel_states)
      {
        ...
        {
          //cornering_ratio = slip_degrees / handling.full_slide_angle;
...
namespace ts
{
  ...
  {    
    ...
    struct Handling
    {
      ...
      double traction_limit = 40000.0;
      double load_transfer = 0.1;

      double brake_balance = 0.4;
      double downforce_balance = 0.56;
      double steering_balance = 0.0;
                                   ...
                                   double frame_duration)
    {
      ...
      auto front_load_transfer = handling_state.net_force.y * handling.load_transfer;
      auto rear_load_transfer = -handling_state.net_force.y * handling.load_transfer;
      ...
      auto front_cornering = 1.0 - handling.steering_balance;
      auto rear_cornering = handling.steering_balance;
      auto cornering_multiplier = handling.cornering / std::max(front_cornering, rear_cornering);
      front_cornering *= cornering_multiplier;
      rear_cornering *= cornering_multiplier;
      ...
      for (auto p : front_wheel_positions)
      {
        ...
        ws.acceleration = front_acceleration;
        ws.braking = 1.0 - handling.brake_balance;
        ws.cornering = front_cornering;
        ws.antislide = handling.antislide;
        ws.bias = cornering_bias_2d;
        wheel_states.push_back(ws);
      }
      ...
      auto slip_angle = radians(std::atan2(local_velocity.x, -local_velocity.y));
      if (std::abs(slip_angle.degrees()) >= 90.0)
      {
        slip_angle -= degrees(180.0);
        slip_angle.normalize();
      ...
      if (slip_degrees < handling.full_slide_angle)
      {
        ...
        {
          slide_ratio = (slip_degrees - handling.non_slide_angle) /
            (handling.full_slide_angle - handling.non_slide_angle);
          slide_ratio *= slide_ratio;
        }
      }

      auto target_heading = make_vector2(0.0, -1.0);
      if (local_heading.y > 0.0) target_heading = -target_heading;
      auto antislide_direction = normalize(target_heading - local_heading);
      
      // Do not allow a force to make slip angle go through zero.

      auto max_antislide = speed * car.mass() * std::abs(slip_angle.radians()) * inv_num_wheels / frame_duration;

      auto pedal_adjustment = 1.0;
      auto steering_adjustment = 1.0;
      for (auto& ws : wheel_states)
      {
        auto wheel_velocity = local_velocity + car.angular_velocity() * make_vector2(-ws.pos.y, ws.pos.x);
        ws.heading = normalize(wheel_velocity);

        ws.traction_limit *= interpolate_linearly(1.0, handling.sliding_grip, slide_ratio);        

        ws.cornering_force = ws.cornering * turning_rate * ws.traction_limit;
        ws.antislide_force = std::min(ws.antislide * ws.traction_limit * antislide_ratio, max_antislide);
        ws.acceleration_force = ws.acceleration * acceleration_force_1d;
        ws.braking_force = ws.braking * braking_force_1d;        

        auto abs_cornering = std::abs(ws.cornering_force);
        auto longitudinal_force = (std::abs(ws.acceleration_force) + ws.braking_force) * pedal_adjustment;
        auto lateral_force = std::max(abs_cornering * steering_adjustment, ws.antislide_force);

        auto applied_force = make_vector2(lateral_force, longitudinal_force);

        if (magnitude_squared(applied_force) > ws.traction_limit * ws.traction_limit)
        {
          auto reduce_steering = abs_cornering * steering_adjustment > ws.bias.x * ws.traction_limit;
          auto reduce_x = lateral_force > ws.bias.x * ws.traction_limit;
          auto reduce_y = longitudinal_force > ws.bias.y * ws.traction_limit;
          ...
          if (reduce_y)
          {
            if (reduce_x)
            {
              if (reduce_steering)
              {
                steering_adjustment = (ws.traction_limit * ws.bias.x) / abs_cornering;
              }

              pedal_adjustment = (ws.traction_limit * ws.bias.y) / (std::abs(ws.acceleration_force) + ws.braking_force);
            }

            else
            {
              auto rem = ws.traction_limit * ws.traction_limit - lateral_force * lateral_force;
              pedal_adjustment = std::sqrt(rem) / (std::abs(ws.acceleration_force) + ws.braking_force);
            }
          }

          else if (reduce_steering)
          {
            auto rem = ws.traction_limit * ws.traction_limit - longitudinal_force * longitudinal_force;
            steering_adjustment = std::sqrt(rem) / abs_cornering;
          }
        }
      }
      ...
      for (auto& ws : wheel_states)
      {
        ...
        auto relative_pos = ws.pos - car.center_of_mass();

        auto force = make_vector2(0.0, 0.0);
        auto wheel_facing = make_vector2(0.0, -1.0);

        auto antislide_1d = ws.antislide_force;
        if (antislide_1d * antislide_direction.x < 0.0) antislide_1d = -antislide_1d;
                
        
        auto lateral_force = interpolate_linearly(antislide_1d, ws.cornering_force * steering_adjustment,
                                                  ws.cornering * std::abs(turning_rate));

        auto torque = cross_product(relative_pos, make_vector2(lateral_force, 0.0));
        car.apply_torque(torque);

        force += ws.acceleration_force * pedal_adjustment * wheel_facing;
        force += ws.braking_force * pedal_adjustment * -ws.heading;
        
        auto rolling_resistance = (1.0 - slide_ratio) * handling.rolling_drag_coefficient * ws.traction_limit;
        force += rolling_resistance * -ws.heading;

        car.apply_force(force, ws.pos);
        car.apply_force(ws.antislide_force * antislide_direction, car.center_of_mass());
      
        net_force += ws.antislide_force * antislide_direction;
        net_force += force;
      }
