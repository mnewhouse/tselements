...
void main()
{
  out_fragColor = vec4(frag_color.rgb, frag_color.a * texture(u_textureSampler, frag_texCoord).a);
}
)";
    ...
    void Context::Renderer::initialize_fonts()
    {
      ...
      io.Fonts->TexID = reinterpret_cast<void*>(static_cast<std::uintptr_t>(texture));
    ...
    {
      using sf::Keyboard;
      const auto& key_state = Keyboard::isKeyPressed;
      ...
      {
        io.MouseDown[button_index] = mouse_press_events_[button_index] || button_state(button);
        ++button_index;
      }
